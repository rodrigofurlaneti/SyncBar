...
// --- Dashboard & Enhanced Pages -------------------------------------------------------------------

// Financial Summary - para Dashboard
export interface IFoodFinancialSummaryResponse {
  grossTotal: number;
  fees: number;
  netTotal: number;
...
export const getIFoodFinancialSummary = (branchId: number, from?: Date, to?: Date): Promise<IFoodFinancialSummaryResponse> => {
  const params = new URLSearchParams();
  if (from) params.append("from", from.toISOString());
  if (to) params.append("to", to.toISOString());
  const query = params.toString();
  return api<IFoodFinancialSummaryResponse>(`/api/integrations/ifood/financial/branch/${branchId}/summary${query ? `?${query}` : ""}`);
};

// Reviews - para Dashboard
export interface IFoodReviewResponse {
  id: string;
  rating: number;
  message: string;
  customerName: string;
...
export const getIFoodReviews = (
  ...
  const query = params.toString();
  return api<IFoodReviewResponse[]>(`/api/integrations/ifood/reviews/branch/${branchId}${query ? `?${query}` : ""}`);
};

export const respondIFoodReview = (branchId: number, reviewId: string, response: string): Promise<void> =>
  api<void>(`/api/integrations/ifood/reviews/branch/${branchId}/${reviewId}/respond`, {
    method: "POST",
    body: JSON.stringify({ response }),
  });

// Merchant Availability Toggle
export const toggleIFoodMerchantAvailability = (branchId: number, available: boolean): Promise<void> =>
  api<void>(`/api/integrations/ifood/merchants/${branchId}/availability`, {
    method: "PUT",
    body: JSON.stringify({ available }),
  });
...
export const getIFoodMerchantStatusByOperation = (branchId: number, operation: string): Promise<IFoodMerchantStatusByOperationResponse> =>
  api<IFoodMerchantStatusByOperationResponse>(`/api/integrations/ifood/merchants/status/branch/${branchId}/${operation}`);

// Analytics - Order KPIs
export interface IFoodOrderKpisResponse {
  currentPage: number;
  buckets: string[]; // JSON bruto de cada bucket agregado
}

export const getIFoodOrderKpis = (branchId: number, periodStart?: Date, periodEnd?: Date, page = 1): Promise<IFoodOrderKpisResponse> => {
  const params = new URLSearchParams();
  ...
  const query = params.toString();
  return api<IFoodOrderKpisResponse>(`/api/integrations/ifood/analytics/kpis/branch/${branchId}${query ? `?${query}` : ""}`);
};
