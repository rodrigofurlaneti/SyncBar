...
type StatusDisplay = ReturnType<typeof formatMerchantAvailability>;
...
type OperationStatusData = {
  available: boolean;
  state?: string | null;
  validations?: ValidationItem[] | null;
};
...
function getOperationButtonStyle(op: string, selectedOperation: string | null): CSSProperties {
...
// Resumo (badges) de erros/avisos exibido no card principal de status.
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12 }}>
      {errorCount > 0 && (
        ...
        >
          <p style={{ fontSize: "0.85rem", margin: 0, fontWeight: 600, color }}>
...
// Card principal com o estado geral da loja no iFood.
function StatusHeaderCard({
  ...
  return (
    ...
    >
      ...
      <ValidationSummaryBadges errorCount={errorCount} warningCount={warningCount} color={statusDisplay.color} />
    </div>
  );
}

// ABA 1: lista de validações gerais da loja.
function GeneralValidationsTab({ validations }: { validations: ValidationItem[] }) {
...
// Grade de botões de seleção de operação (Delivery/Retirada/Consumo local).
function OperationButtons({
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginBottom: 24 }}>
      {operations.map((op) => (
        <button key={op} onClick={() => onSelect(op)} style={getOperationButtonStyle(op, selectedOperation)}>
...
// Lista de validações específicas de uma operação selecionada.
...
// Detalhes (status + validações) da operação selecionada, cobrindo loading/erro/dados.
function OperationStatusDetails({
  ...
  isError: boolean;
  error: unknown;
  operationData: OperationStatusData | undefined;
  ...
  if (isError) {
    return <QueryError error={error} what={`status da operação ${selectedOperation}`} />;
  }
...
// ABA 2: seleção de operação + detalhes da operação escolhida.
function OperationsTab({
  ...
  operationStatusQuery: { isLoading: boolean; isError: boolean; error: unknown };
  operationData: OperationStatusData | undefined;
}) {
  return (
  ...
  return (
    <main style={{ padding: 22, maxWidth: 1200, margin: "0 auto" }}>
      ...
      >
        ...
        {activeTab === "geral" && <GeneralValidationsTab validations={validations} />}
