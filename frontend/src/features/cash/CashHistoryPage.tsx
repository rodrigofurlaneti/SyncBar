...
export function CashHistoryPage() {
    ...
    const reviewMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            Swal.fire({
                title: "Sucesso!",
                text: "Caixa marcado como conferido.",
                icon: "success",
                timer: 2000,
        ...
        onError: (e) => {
            const msg = e instanceof ApiError ? e.message : "Falha ao conferir.";
            setError(msg);
        ...
        <main style={{ padding: 22, maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div className="ticket rise rise-1" data-testid="history-list">
                {sessions.map((session) => {
                    ...
                    return (
                        <div className="ticket-row" key={session.id} style={{ flexWrap: "wrap", gap: 10 }} data-testid={`session-row-${session.id}`}>
                            <div style={{ display: "grid", gap: 3, minWidth: 220 }}>
                                <span>
                                    ...
                                    {session.cashSessionStatusId === CashSessionStatus.Conferido && (
                                        <span style={{ color: "var(--ok)", fontSize: "0.78rem", marginLeft: 8 }} data-testid={`status-conferido-${session.id}`}>
                                            ✓ conferido
                                        </span>
                                    )}
                                </span>
                            ...
                            <div style={{ display: "flex", gap: 10, alignItems: "center", marginLeft: "auto" }}>
                                ...
                                {session.cashSessionStatusId === CashSessionStatus.Fechado && (
                                    <button
                                        ...
                                        data-testid={`review-btn-${session.id}`}
                                        onClick={async () => {
                                            const { isConfirmed } = await Swal.fire({
                                                title: "Marcar como conferido?",
                                                text: `Confirmar conferência da sessão #${session.id}?`,
                                                icon: "question",
                                                showCancelButton: true,
                                                confirmButtonText: "Sim, conferido",
                                                cancelButtonText: "Cancelar"
                                            });

                                            if (isConfirmed) {
                                                reviewMutation.mutate(session.id);
                                            }
                                        }}
                                    >
                                        Marcar conferido
