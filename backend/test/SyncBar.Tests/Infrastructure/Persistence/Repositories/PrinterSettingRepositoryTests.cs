...
{
    public sealed class PrinterSettingRepositoryTests : RepositoryTestBase
    {
        private readonly PrinterSettingRepository _repository;

        public PrinterSettingRepositoryTests()
        {
            _repository = new PrinterSettingRepository(Context);
        }

        private static PrinterSetting CreateSetting(long branchId = 1) =>
            PrinterSetting.Create(branchId, printOrdersEnabled: true, printBillsEnabled: true).Value;

        private async Task<PrinterSetting> SeedAsync(PrinterSetting setting)
        {
        ...
        {
            var setting = CreateSetting(branchId: 7);

            await _repository.AddAsync(setting);
            await Context.SaveChangesAsync();

            var persisted = await Context.Set<PrinterSetting>()
                .AsNoTracking()
