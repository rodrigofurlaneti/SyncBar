...
public sealed class ListIfoodOptionGroupsQueryHandlerTests
{
    ...
    private readonly ListIfoodOptionGroupsQueryHandler _handler;

    public ListIfoodOptionGroupsQueryHandlerTests()
    {
        _handler = new ListIfoodOptionGroupsQueryHandler(
    ...
    {
        var query = new ListIfoodOptionGroupsQuery(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnOptionGroupsFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new ListIfoodOptionGroupsQuery(1);
        _catalogClient.ListOptionGroupsAsync("token-1", "MERCH-1", false, null, Arg.Any<CancellationToken>())
            .Returns(new IfoodOptionGroupListResult(false, [], "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.OptionGroupsFetchFailed");
    }

    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnMappedGroups()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new ListIfoodOptionGroupsQuery(1, true, "ifood");
        _catalogClient.ListOptionGroupsAsync("token-1", "MERCH-1", true, "ifood", Arg.Any<CancellationToken>())
            .Returns(new IfoodOptionGroupListResult(true, [new IfoodOptionGroupDto("group-1", "Bebidas", "ext-1", "AVAILABLE", 1)], null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Should().ContainSingle(g => g.Id == "group-1");
    }
}
