...
public sealed class SetIfoodOptionStatusCommandHandlerTests
{
    ...
    private readonly SetIfoodOptionStatusCommandHandler _handler;

    public SetIfoodOptionStatusCommandHandlerTests()
    {
        _handler = new SetIfoodOptionStatusCommandHandler(
    ...
    {
        var command = new SetIfoodOptionStatusCommand(1, Guid.NewGuid(), false, null);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnSetOptionStatusFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodOptionStatusCommand(1, Guid.NewGuid(), false, null);
        _catalogClient.SetOptionStatusAsync("token-1", "MERCH-1", command.OptionId, false, null, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.SetOptionStatusFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodOptionStatusCommand(1, Guid.NewGuid(), true, "parent-1");
        _catalogClient.SetOptionStatusAsync("token-1", "MERCH-1", command.OptionId, true, "parent-1", Arg.Any<CancellationToken>())
