...
public sealed class GetIfoodItemFlatQueryHandlerTests
{
    ...
    private readonly GetIfoodItemFlatQueryHandler _handler;

    public GetIfoodItemFlatQueryHandlerTests()
    {
        _handler = new GetIfoodItemFlatQueryHandler(
    ...
    {
        var query = new GetIfoodItemFlatQuery(1, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnItemFlatFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodItemFlatQuery(1, Guid.NewGuid());
        _catalogClient.GetItemFlatAsync("token-1", "MERCH-1", query.ItemId, Arg.Any<CancellationToken>())
            .Returns(new IfoodItemFlatResult(false, null, null, null, null, null, null, "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.ItemFlatFetchFailed");
    }

    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnMappedItem()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodItemFlatQuery(1, Guid.NewGuid());
        _catalogClient.GetItemFlatAsync("token-1", "MERCH-1", query.ItemId, Arg.Any<CancellationToken>())
            .Returns(new IfoodItemFlatResult(true, "item-1", "AVAILABLE", 10m, "ext-1", "cat-1", "{}", null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.ItemId.Should().Be("item-1");
        result.Value.PriceValue.Should().Be(10m);
    }
}
