...
public sealed class GetIfoodCategoryQueryHandlerTests
{
    ...
    private readonly GetIfoodCategoryQueryHandler _handler;

    public GetIfoodCategoryQueryHandlerTests()
    {
        _handler = new GetIfoodCategoryQueryHandler(
    ...
    {
        var query = new GetIfoodCategoryQuery(1, "catalog-1", "cat-1");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnCategoryFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodCategoryQuery(1, "catalog-1", "cat-1");
        _catalogClient.GetCategoryAsync("token-1", "MERCH-1", "catalog-1", "cat-1", false, Arg.Any<CancellationToken>())
            .Returns(new IfoodCategoryDetailResult(false, null, null, "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.CategoryFetchFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldReturnMappedCategory()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodCategoryQuery(1, "catalog-1", "cat-1", true);
        var category = new IfoodCategoryDto("cat-1", 1, "Bebidas", "ext-1", "AVAILABLE", "DEFAULT");
        _catalogClient.GetCategoryAsync("token-1", "MERCH-1", "catalog-1", "cat-1", true, Arg.Any<CancellationToken>())
