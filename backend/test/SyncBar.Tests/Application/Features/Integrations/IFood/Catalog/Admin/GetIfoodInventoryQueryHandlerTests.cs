...
public sealed class GetIfoodInventoryQueryHandlerTests
{
    ...
    private readonly GetIfoodInventoryQueryHandler _handler;

    public GetIfoodInventoryQueryHandlerTests()
    {
        _handler = new GetIfoodInventoryQueryHandler(
    ...
    {
        var query = new GetIfoodInventoryQuery(1, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnInventoryFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodInventoryQuery(1, Guid.NewGuid());
        _catalogClient.GetInventoryAsync("token-1", "MERCH-1", query.ProductId, Arg.Any<CancellationToken>())
            .Returns(new IfoodInventoryResult(false, null, "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.InventoryFetchFailed");
    }

    [Fact]
    public async Task Handle_NullInventoryEvenWhenSuccess_ShouldReturnInventoryFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodInventoryQuery(1, Guid.NewGuid());
        _catalogClient.GetInventoryAsync("token-1", "MERCH-1", query.ProductId, Arg.Any<CancellationToken>())
            .Returns(new IfoodInventoryResult(true, null, null));
    ...
    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnMappedInventory()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodInventoryQuery(1, Guid.NewGuid());
        _catalogClient.GetInventoryAsync("token-1", "MERCH-1", query.ProductId, Arg.Any<CancellationToken>())
            .Returns(new IfoodInventoryResult(true, new IfoodInventoryDto("prod-1", "owner-1", 10, true), null));
