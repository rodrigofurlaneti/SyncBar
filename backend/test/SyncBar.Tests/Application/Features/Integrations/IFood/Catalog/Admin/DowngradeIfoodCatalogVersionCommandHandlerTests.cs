...
public sealed class DowngradeIfoodCatalogVersionCommandHandlerTests
{
    ...
    private readonly DowngradeIfoodCatalogVersionCommandHandler _handler;

    public DowngradeIfoodCatalogVersionCommandHandlerTests()
    {
        _handler = new DowngradeIfoodCatalogVersionCommandHandler(
    ...
    {
        var command = new DowngradeIfoodCatalogVersionCommand(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDowngradeVersionFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DowngradeIfoodCatalogVersionCommand(1);
        _catalogClient.DowngradeVersionAsync("token-1", "MERCH-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DowngradeVersionFailed");
    }
