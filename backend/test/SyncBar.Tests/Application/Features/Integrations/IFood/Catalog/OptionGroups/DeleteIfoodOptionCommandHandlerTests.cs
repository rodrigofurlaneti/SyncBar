...
public sealed class DeleteIfoodOptionCommandHandlerTests
{
    ...
    private readonly DeleteIfoodOptionCommandHandler _handler;

    public DeleteIfoodOptionCommandHandlerTests()
    {
        _handler = new DeleteIfoodOptionCommandHandler(
    ...
    {
        var command = new DeleteIfoodOptionCommand(1, Guid.NewGuid(), Guid.NewGuid(), null);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDeleteOptionFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DeleteIfoodOptionCommand(1, Guid.NewGuid(), Guid.NewGuid(), null);
        _catalogClient.DeleteOptionAsync("token-1", "MERCH-1", command.OptionGroupId, command.ProductId, null, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DeleteOptionFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DeleteIfoodOptionCommand(1, Guid.NewGuid(), Guid.NewGuid(), "context-1");
        _catalogClient.DeleteOptionAsync("token-1", "MERCH-1", command.OptionGroupId, command.ProductId, "context-1", Arg.Any<CancellationToken>())
