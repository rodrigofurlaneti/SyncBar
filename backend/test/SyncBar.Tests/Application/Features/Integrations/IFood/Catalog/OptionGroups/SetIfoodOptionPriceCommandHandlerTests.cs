...
public sealed class SetIfoodOptionPriceCommandHandlerTests
{
    ...
    private readonly SetIfoodOptionPriceCommandHandler _handler;

    public SetIfoodOptionPriceCommandHandlerTests()
    {
        _handler = new SetIfoodOptionPriceCommandHandler(
    ...
    {
        var command = new SetIfoodOptionPriceCommand(1, Guid.NewGuid(), 5m, null, null);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnSetOptionPriceFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodOptionPriceCommand(1, Guid.NewGuid(), 5m, null, null);
        _catalogClient.SetOptionPriceAsync("token-1", "MERCH-1", command.OptionId, 5m, null, null, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.SetOptionPriceFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodOptionPriceCommand(1, Guid.NewGuid(), 5m, 6m, "parent-1");
        _catalogClient.SetOptionPriceAsync("token-1", "MERCH-1", command.OptionId, 5m, 6m, "parent-1", Arg.Any<CancellationToken>())
