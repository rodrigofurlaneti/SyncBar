...
public sealed class UpgradeIfoodCatalogVersionCommandHandlerTests
{
    ...
    private readonly UpgradeIfoodCatalogVersionCommandHandler _handler;

    public UpgradeIfoodCatalogVersionCommandHandlerTests()
    {
        _handler = new UpgradeIfoodCatalogVersionCommandHandler(
    ...
    {
        var command = new UpgradeIfoodCatalogVersionCommand(1, true);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnUpgradeVersionFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new UpgradeIfoodCatalogVersionCommand(1, true);
        _catalogClient.UpgradeVersionAsync("token-1", "MERCH-1", true, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.UpgradeVersionFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new UpgradeIfoodCatalogVersionCommand(1, null);
        _catalogClient.UpgradeVersionAsync("token-1", "MERCH-1", null, Arg.Any<CancellationToken>())
