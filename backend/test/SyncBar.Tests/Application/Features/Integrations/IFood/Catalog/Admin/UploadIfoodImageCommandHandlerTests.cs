...
public sealed class UploadIfoodImageCommandHandlerTests
{
    ...
    private readonly UploadIfoodImageCommandHandler _handler;

    public UploadIfoodImageCommandHandlerTests()
    {
        _handler = new UploadIfoodImageCommandHandler(
    ...
    {
        var command = new UploadIfoodImageCommand(1, "{}");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnUploadImageFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new UploadIfoodImageCommand(1, "{\"image\":\"base64\"}");
        _catalogClient.UploadImageAsync("token-1", "MERCH-1", "{\"image\":\"base64\"}", Arg.Any<CancellationToken>())
            .Returns(new IfoodImageUploadResult(false, null, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.UploadImageFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldReturnRawPayload()
    {
        ...
        var command = new UploadIfoodImageCommand(1, "{\"image\":\"base64\"}");
        _catalogClient.UploadImageAsync("token-1", "MERCH-1", "{\"image\":\"base64\"}", Arg.Any<CancellationToken>())
            .Returns(new IfoodImageUploadResult(true, "{\"imageId\":\"1\"}", null));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.RawPayload.Should().Be("{\"imageId\":\"1\"}");
    }
}
