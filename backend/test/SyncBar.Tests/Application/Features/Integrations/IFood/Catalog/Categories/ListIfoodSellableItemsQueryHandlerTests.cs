...
public sealed class ListIfoodSellableItemsQueryHandlerTests
{
    ...
    private readonly ListIfoodSellableItemsQueryHandler _handler;

    public ListIfoodSellableItemsQueryHandlerTests()
    {
        _handler = new ListIfoodSellableItemsQueryHandler(
    ...
    {
        var query = new ListIfoodSellableItemsQuery(1, "group-1");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnSellableItemsFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new ListIfoodSellableItemsQuery(1, "group-1");
        _catalogClient.ListSellableItemsAsync("token-1", "MERCH-1", "group-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodSellableItemsResult(false, [], "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.SellableItemsFetchFailed");
    }

    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnMappedItems()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new ListIfoodSellableItemsQuery(1, "group-1");
        _catalogClient.ListSellableItemsAsync("token-1", "MERCH-1", "group-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodSellableItemsResult(true, [new IfoodSellableItemDto("item-1", "cat-1", "Refrigerante", "ext-1", "789", 10m)], null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Should().ContainSingle(i => i.ItemId == "item-1" && i.ItemPriceValue == 10m);
    }
}
