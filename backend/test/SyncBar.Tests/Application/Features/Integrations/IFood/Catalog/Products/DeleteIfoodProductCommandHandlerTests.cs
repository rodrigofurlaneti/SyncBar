...
public sealed class DeleteIfoodProductCommandHandlerTests
{
    ...
    private readonly DeleteIfoodProductCommandHandler _handler;

    public DeleteIfoodProductCommandHandlerTests()
    {
        _handler = new DeleteIfoodProductCommandHandler(
    ...
    {
        var command = new DeleteIfoodProductCommand(1, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDeleteProductFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DeleteIfoodProductCommand(1, Guid.NewGuid());
        _catalogClient.DeleteProductAsync("token-1", "MERCH-1", command.ProductId, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DeleteProductFailed");
    }
