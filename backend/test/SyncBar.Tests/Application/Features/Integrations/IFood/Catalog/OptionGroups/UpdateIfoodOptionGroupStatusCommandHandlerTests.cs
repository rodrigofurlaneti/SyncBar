...
public sealed class UpdateIfoodOptionGroupStatusCommandHandlerTests
{
    ...
    private readonly UpdateIfoodOptionGroupStatusCommandHandler _handler;

    public UpdateIfoodOptionGroupStatusCommandHandlerTests()
    {
        _handler = new UpdateIfoodOptionGroupStatusCommandHandler(
    ...
    {
        var command = new UpdateIfoodOptionGroupStatusCommand(1, Guid.NewGuid(), false);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnUpdateOptionGroupStatusFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new UpdateIfoodOptionGroupStatusCommand(1, Guid.NewGuid(), false);
        _catalogClient.UpdateOptionGroupStatusAsync("token-1", "MERCH-1", command.OptionGroupId, false, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.UpdateOptionGroupStatusFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new UpdateIfoodOptionGroupStatusCommand(1, Guid.NewGuid(), true);
        _catalogClient.UpdateOptionGroupStatusAsync("token-1", "MERCH-1", command.OptionGroupId, true, Arg.Any<CancellationToken>())
