...
public sealed class SetIfoodItemPriceCommandHandlerTests
{
    ...
    private readonly SetIfoodItemPriceCommandHandler _handler;

    public SetIfoodItemPriceCommandHandlerTests()
    {
        _handler = new SetIfoodItemPriceCommandHandler(
    ...
    {
        var command = new SetIfoodItemPriceCommand(1, Guid.NewGuid(), 10m, null, null);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnSetItemPriceFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodItemPriceCommand(1, Guid.NewGuid(), 10m, null, null);
        _catalogClient.SetItemPriceAsync("token-1", "MERCH-1", command.ItemId, 10m, null, null, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.SetItemPriceFailed");
    }

    [Fact]
    public async Task Handle_ValidRequestWithPriceByCatalog_ShouldMapAndSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodItemPriceCommand(1, Guid.NewGuid(), 10m, 12m, [new IfoodItemPriceByCatalogInput(9m, "ifood", 11m)]);
        _catalogClient.SetItemPriceAsync(
            "token-1", "MERCH-1", command.ItemId, 10m, 12m,
            Arg.Is<IReadOnlyCollection<IfoodItemPriceByCatalog>>(l => l.Count == 1 && l.First().Value == 9m),
