...
public sealed class DisassociateIfoodOptionGroupCommandHandlerTests
{
    ...
    private readonly DisassociateIfoodOptionGroupCommandHandler _handler;

    public DisassociateIfoodOptionGroupCommandHandlerTests()
    {
        _handler = new DisassociateIfoodOptionGroupCommandHandler(
    ...
    {
        var command = new DisassociateIfoodOptionGroupCommand(1, Guid.NewGuid(), Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDisassociateOptionGroupFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DisassociateIfoodOptionGroupCommand(1, Guid.NewGuid(), Guid.NewGuid());
        _catalogClient.DisassociateOptionGroupFromProductAsync("token-1", "MERCH-1", command.OptionGroupId, command.ProductId, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DisassociateOptionGroupFailed");
    }
