...
public sealed class DeleteIfoodInventoryBatchCommandHandlerTests
{
    ...
    private readonly DeleteIfoodInventoryBatchCommandHandler _handler;

    public DeleteIfoodInventoryBatchCommandHandlerTests()
    {
        _handler = new DeleteIfoodInventoryBatchCommandHandler(
    ...
    {
        var command = new DeleteIfoodInventoryBatchCommand(1, [Guid.NewGuid()]);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDeleteInventoryBatchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DeleteIfoodInventoryBatchCommand(1, [Guid.NewGuid()]);
        _catalogClient.DeleteInventoryBatchAsync("token-1", "MERCH-1", Arg.Any<IReadOnlyCollection<Guid>>(), Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DeleteInventoryBatchFailed");
    }
