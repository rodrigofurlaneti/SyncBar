...
public sealed class GetIfoodCatalogsQueryHandlerTests
{
    ...
    private readonly GetIfoodCatalogsQueryHandler _handler;

    public GetIfoodCatalogsQueryHandlerTests()
    {
        _handler = new GetIfoodCatalogsQueryHandler(
    ...
    {
        var query = new GetIfoodCatalogsQuery(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnCatalogsFetchFailed()
    {
    ...
    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnMappedCatalogs()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodCatalogsQuery(1);
        _catalogClient.GetCatalogsAsync("token-1", "MERCH-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogsListResult(true, [new IfoodCatalogSummaryDto("cat-1", "AVAILABLE", ["DELIVERY"], "group-1", DateTime.UtcNow)], null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Should().ContainSingle(c => c.CatalogId == "cat-1");
    }
}
