...
public sealed class CreateIfoodProductCommandHandlerTests
{
    ...
    private readonly CreateIfoodProductCommandHandler _handler;

    public CreateIfoodProductCommandHandlerTests()
    {
        _handler = new CreateIfoodProductCommandHandler(
    ...
    private static CreateIfoodProductCommand ValidCommand() =>
        new(1, null, "Refrigerante", "descrição", null, "ext-1", "789", null, null);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnCreateProductFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = ValidCommand();
        _catalogClient.CreateProductAsync("token-1", "MERCH-1", Arg.Any<IfoodUpsertProductRequest>(), Arg.Any<CancellationToken>())
            .Returns(new IfoodProductDetailResult(false, null, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.CreateProductFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldReturnMappedProduct()
    {
        ...
        _catalogClient.CreateProductAsync(
            "token-1", "MERCH-1",
            Arg.Is<IfoodUpsertProductRequest>(r => r.Name == "Refrigerante" && r.ExternalCode == "ext-1"),
