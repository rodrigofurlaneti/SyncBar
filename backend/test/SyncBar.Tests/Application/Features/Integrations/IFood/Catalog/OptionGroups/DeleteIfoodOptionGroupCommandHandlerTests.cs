...
public sealed class DeleteIfoodOptionGroupCommandHandlerTests
{
    ...
    private readonly DeleteIfoodOptionGroupCommandHandler _handler;

    public DeleteIfoodOptionGroupCommandHandlerTests()
    {
        _handler = new DeleteIfoodOptionGroupCommandHandler(
    ...
    {
        var command = new DeleteIfoodOptionGroupCommand(1, Guid.NewGuid());
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDeleteOptionGroupFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DeleteIfoodOptionGroupCommand(1, Guid.NewGuid());
        _catalogClient.DeleteOptionGroupAsync("token-1", "MERCH-1", command.OptionGroupId, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DeleteOptionGroupFailed");
    }
