...
public sealed class SetIfoodOptionExternalCodeCommandHandlerTests
{
    ...
    private readonly SetIfoodOptionExternalCodeCommandHandler _handler;

    public SetIfoodOptionExternalCodeCommandHandlerTests()
    {
        _handler = new SetIfoodOptionExternalCodeCommandHandler(
    ...
    {
        var command = new SetIfoodOptionExternalCodeCommand(1, Guid.NewGuid(), "ext-1", null);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnSetOptionExternalCodeFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodOptionExternalCodeCommand(1, Guid.NewGuid(), "ext-1", null);
        _catalogClient.SetOptionExternalCodeAsync("token-1", "MERCH-1", command.OptionId, "ext-1", null, Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.SetOptionExternalCodeFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new SetIfoodOptionExternalCodeCommand(1, Guid.NewGuid(), "ext-1", "parent-1");
        _catalogClient.SetOptionExternalCodeAsync("token-1", "MERCH-1", command.OptionId, "ext-1", "parent-1", Arg.Any<CancellationToken>())
