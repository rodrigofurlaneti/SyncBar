...
public sealed class DeleteIfoodCategoryCommandHandlerTests
{
    ...
    private readonly DeleteIfoodCategoryCommandHandler _handler;

    public DeleteIfoodCategoryCommandHandlerTests()
    {
        _handler = new DeleteIfoodCategoryCommandHandler(
    ...
    {
        var command = new DeleteIfoodCategoryCommand(1, "cat-1");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDeleteCategoryFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new DeleteIfoodCategoryCommand(1, "cat-1");
        _catalogClient.DeleteCategoryAsync("token-1", "MERCH-1", "cat-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodCatalogActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.DeleteCategoryFailed");
    }
