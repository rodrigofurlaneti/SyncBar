...
public sealed class ListIfoodCategoryItemsQueryHandlerTests
{
    ...
    private readonly ListIfoodCategoryItemsQueryHandler _handler;

    public ListIfoodCategoryItemsQueryHandlerTests()
    {
        _handler = new ListIfoodCategoryItemsQueryHandler(
    ...
    {
        var query = new ListIfoodCategoryItemsQuery(1, "cat-1");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnCategoryItemsFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new ListIfoodCategoryItemsQuery(1, "cat-1");
        _catalogClient.ListCategoryItemsAsync("token-1", "MERCH-1", "cat-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodCategoryItemsResult(false, null, "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.CategoryItemsFetchFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldReturnRawPayload()
    {
        ...
        var query = new ListIfoodCategoryItemsQuery(1, "cat-1");
        _catalogClient.ListCategoryItemsAsync("token-1", "MERCH-1", "cat-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodCategoryItemsResult(true, "[{\"id\":\"item-1\"}]", null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.RawPayload.Should().Be("[{\"id\":\"item-1\"}]");
    }
}
