...
public sealed class ListIfoodProductsByExternalCodeQueryHandlerTests
{
    ...
    private readonly ListIfoodProductsByExternalCodeQueryHandler _handler;

    public ListIfoodProductsByExternalCodeQueryHandlerTests()
    {
        _handler = new ListIfoodProductsByExternalCodeQueryHandler(
    ...
    {
        var query = new ListIfoodProductsByExternalCodeQuery(1, "ext-1");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnProductsByExternalCodeFetchFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new ListIfoodProductsByExternalCodeQuery(1, "ext-1");
        _catalogClient.ListProductsByExternalCodeAsync("token-1", "MERCH-1", "ext-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodProductListResult(false, [], "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodCatalog.ProductsByExternalCodeFetchFailed");
    }
