...
public sealed class StartIFoodOrderPreparationCommandHandlerTests
{
    ...
    private StartIFoodOrderPreparationCommandHandler CreateSut() =>
    ...
    public async Task Handle_WhenIFoodOrderNotFound_ShouldFailWithNotFound()
    {
        ...
        var result = await sut.Handle(new StartIFoodOrderPreparationCommand(99), CancellationToken.None);
    ...
    public async Task Handle_WhenBranchNotFound_ShouldFailWithBranchNotFound()
    {
        ...
        var result = await sut.Handle(new StartIFoodOrderPreparationCommand(1), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenIFoodRejectsStartPreparation_ShouldFailAndNotChangeStatus()
    {
        var ifoodOrder = CreateIFoodOrder();
        GivenAConnectedBranchWithValidToken(ifoodOrder);
        _orderClient.StartPreparationAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
            .Returns(new IFoodOrderActionResult(false, "Pedido já cancelado no iFood."));
        var sut = CreateSut();
        ...
        result.Error.Message.Should().Be("Pedido já cancelado no iFood.");
        ifoodOrder.Status.Should().Be(IFoodOrderStatuses.Placed);
    }

    [Fact]
    public async Task Handle_WhenIFoodAcceptsStartPreparation_ShouldSucceedAndUpdateStatus()
    {
        var ifoodOrder = CreateIFoodOrder();
        GivenAConnectedBranchWithValidToken(ifoodOrder);
        _orderClient.StartPreparationAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
        ...
        ifoodOrder.Status.Should().Be(IFoodOrderStatuses.PreparationStarted);
        ifoodOrder.UpdatedAt.Should().NotBeNull();
        await _orderClient.Received(1).StartPreparationAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>());
    }
}
