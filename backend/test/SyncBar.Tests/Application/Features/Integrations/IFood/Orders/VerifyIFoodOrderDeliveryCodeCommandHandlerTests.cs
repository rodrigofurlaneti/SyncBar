...
public sealed class VerifyIFoodOrderDeliveryCodeCommandHandlerTests
{
    ...
    private VerifyIFoodOrderDeliveryCodeCommandHandler CreateSut() =>
    ...
    public async Task Handle_WhenIFoodOrderNotFound_ShouldFail()
    {
        ...
        var result = await sut.Handle(new VerifyIFoodOrderDeliveryCodeCommand(99, "1234"), CancellationToken.None);
    ...
    public async Task Handle_WhenBranchNotFound_ShouldFail()
    {
        ...
        var result = await sut.Handle(new VerifyIFoodOrderDeliveryCodeCommand(1, "1234"), CancellationToken.None);
    ...
    public async Task Handle_WhenIFoodRequestItselfFails_ShouldFailWithDefaultMessage()
    {
        ...
        _orderClient.VerifyOrderDeliveryCodeAsync(ValidToken, IFoodOrderExternalId, "1234", Arg.Any<CancellationToken>())
        ...
        result.Error.Message.Should().Be("Falha ao verificar o código de entrega no iFood.");
    }
    ...
    {
        GivenAConnectedBranchWithValidToken(CreateIFoodOrder());
        _orderClient.VerifyOrderDeliveryCodeAsync(ValidToken, IFoodOrderExternalId, "0000", Arg.Any<CancellationToken>())
            .Returns(new IFoodPickupValidationResult(true, false, null));
        var sut = CreateSut();

        var result = await sut.Handle(new VerifyIFoodOrderDeliveryCodeCommand(1, "0000"), CancellationToken.None);
