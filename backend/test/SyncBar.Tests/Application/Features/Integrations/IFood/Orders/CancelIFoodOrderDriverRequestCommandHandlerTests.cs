...
public sealed class CancelIFoodOrderDriverRequestCommandHandlerTests
{
    ...
    private CancelIFoodOrderDriverRequestCommandHandler CreateSut() =>
    ...
    public async Task Handle_WhenIFoodOrderNotFound_ShouldFail()
    {
        ...
        var result = await sut.Handle(new CancelIFoodOrderDriverRequestCommand(99), CancellationToken.None);
    ...
    public async Task Handle_WhenBranchNotFound_ShouldFail()
    {
        ...
        var result = await sut.Handle(new CancelIFoodOrderDriverRequestCommand(1), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenIFoodRejectsTheCancellation_ShouldFailWithDefaultMessage()
    {
        GivenAConnectedBranchWithValidToken(CreateIFoodOrder());
        _orderClient.CancelOrderRequestDriverAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
        ...
        result.IsFailure.Should().BeTrue();
        result.Error.Message.Should().Be("Falha ao cancelar a solicitação de entregador no iFood.");
    }

    [Fact]
    public async Task Handle_WhenIFoodAcceptsTheCancellation_ShouldSucceed()
    {
        GivenAConnectedBranchWithValidToken(CreateIFoodOrder());
        _orderClient.CancelOrderRequestDriverAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
        ...
        await _orderClient.Received(1).CancelOrderRequestDriverAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>());
    }
}
