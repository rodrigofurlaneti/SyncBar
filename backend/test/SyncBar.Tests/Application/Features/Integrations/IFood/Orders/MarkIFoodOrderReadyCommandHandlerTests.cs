...
public sealed class MarkIFoodOrderReadyCommandHandlerTests
{
    ...
    // TimeProvider.GetLocalNow() is not virtual, so it can't be stubbed with NSubstitute — the
    // real system clock is used here on purpose; assertions below only check that a status
    // transition happened, not the exact timestamp.
    private MarkIFoodOrderReadyCommandHandler CreateSut() =>
        new(_ifoodOrderRepository, _branchRepository, _tokenProvider, _orderClient,
    ...
    public async Task Handle_WhenIFoodOrderNotFound_ShouldFailWithNotFound()
    {
        ...
        var result = await sut.Handle(new MarkIFoodOrderReadyCommand(99), CancellationToken.None);
    ...
    public async Task Handle_WhenBranchNotFound_ShouldFailWithBranchNotFound()
    {
        ...
        var result = await sut.Handle(new MarkIFoodOrderReadyCommand(1), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenIFoodRejectsReadyToPickup_ShouldFailAndNotChangeStatus()
    {
        var ifoodOrder = CreateIFoodOrder();
        GivenAConnectedBranchWithValidToken(ifoodOrder);
        _orderClient.ReadyToPickupAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
            .Returns(new IFoodOrderActionResult(false, "Pedido já concluído no iFood."));
        var sut = CreateSut();
        ...
        result.Error.Message.Should().Be("Pedido já concluído no iFood.");
        ifoodOrder.Status.Should().Be(IFoodOrderStatuses.Placed);
    }

    [Fact]
    public async Task Handle_WhenIFoodAcceptsReadyToPickup_ShouldSucceedAndUpdateStatus()
    {
        var ifoodOrder = CreateIFoodOrder();
        GivenAConnectedBranchWithValidToken(ifoodOrder);
        _orderClient.ReadyToPickupAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
        ...
        ifoodOrder.Status.Should().Be(IFoodOrderStatuses.ReadyToPickup);
        ifoodOrder.UpdatedAt.Should().NotBeNull();
        await _orderClient.Received(1).ReadyToPickupAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>());
    }
}
