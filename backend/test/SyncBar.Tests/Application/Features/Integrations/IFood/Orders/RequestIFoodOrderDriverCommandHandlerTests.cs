...
public sealed class RequestIFoodOrderDriverCommandHandlerTests
{
    ...
    private RequestIFoodOrderDriverCommandHandler CreateSut() =>
    ...
    public async Task Handle_WhenIFoodOrderNotFound_ShouldFail()
    {
        ...
        var result = await sut.Handle(new RequestIFoodOrderDriverCommand(99), CancellationToken.None);
    ...
    public async Task Handle_WhenBranchNotFound_ShouldFail()
    {
        ...
        var result = await sut.Handle(new RequestIFoodOrderDriverCommand(1), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenIFoodRejectsTheRequest_ShouldFailWithDefaultMessage()
    {
        GivenAConnectedBranchWithValidToken(CreateIFoodOrder());
        _orderClient.RequestOrderDriverAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
        ...
        result.IsFailure.Should().BeTrue();
        result.Error.Message.Should().Be("Falha ao solicitar entregador no iFood.");
    }

    [Fact]
    public async Task Handle_WhenIFoodAcceptsTheRequest_ShouldSucceed()
    {
        GivenAConnectedBranchWithValidToken(CreateIFoodOrder());
        _orderClient.RequestOrderDriverAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>())
        ...
        await _orderClient.Received(1).RequestOrderDriverAsync(ValidToken, IFoodOrderExternalId, Arg.Any<CancellationToken>());
    }
}
