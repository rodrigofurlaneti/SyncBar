...
public sealed class CancelIfoodOrderShippingDriverCommandHandlerTests
{
    ...
    private readonly CancelIfoodOrderShippingDriverCommandHandler _handler;

    public CancelIfoodOrderShippingDriverCommandHandlerTests()
    {
        _handler = new CancelIfoodOrderShippingDriverCommandHandler(
    ...
    {
        var command = new CancelIfoodOrderShippingDriverCommand(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnCancelDriverFailed()
    {
        ...
        _shippingClient.CancelDriverForOrderAsync("token-1", "ifood-order-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodShippingActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodShipping.CancelDriverFailed");
    }
