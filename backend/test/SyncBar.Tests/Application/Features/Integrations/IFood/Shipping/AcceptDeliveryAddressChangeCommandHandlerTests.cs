...
public sealed class AcceptDeliveryAddressChangeCommandHandlerTests
{
    ...
    private readonly AcceptDeliveryAddressChangeCommandHandler _handler;

    public AcceptDeliveryAddressChangeCommandHandlerTests()
    {
        _handler = new AcceptDeliveryAddressChangeCommandHandler(
    ...
    {
        var command = new AcceptDeliveryAddressChangeCommand(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnAcceptAddressChangeFailed()
    {
        var order = CreateOrder();
        var branch = CreateBranch();
        var command = new AcceptDeliveryAddressChangeCommand(1);
        ...
        _shippingClient.AcceptDeliveryAddressChangeAsync("token-1", "ifood-order-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodShippingActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodShipping.AcceptAddressChangeFailed");
    }
