...
public sealed class GetIfoodSafeDeliveryScoreQueryHandlerTests
{
    ...
    private readonly GetIfoodSafeDeliveryScoreQueryHandler _handler;

    public GetIfoodSafeDeliveryScoreQueryHandlerTests()
    {
        _handler = new GetIfoodSafeDeliveryScoreQueryHandler(
    ...
    {
        var query = new GetIfoodSafeDeliveryScoreQuery(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnSafeDeliveryFailed()
    {
        ...
        _shippingClient.GetSafeDeliveryScoreAsync("token-1", "delivery-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodSafeDeliveryScoreResult(false, "erro remoto", null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodShipping.SafeDeliveryFailed");
    }

    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnScore()
    {
        var delivery = CreateDelivery();
        var branch = CreateBranch();
        var query = new GetIfoodSafeDeliveryScoreQuery(1);
        ...
        _tokenProvider.GetAccessTokenAsync(branch.CompanyId, Arg.Any<CancellationToken>()).Returns("token-1");
        _shippingClient.GetSafeDeliveryScoreAsync("token-1", "delivery-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodSafeDeliveryScoreResult(true, null, "HIGH"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Score.Should().Be("HIGH");
    }
}
