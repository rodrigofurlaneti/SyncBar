...
public sealed class ConfirmUserAddressCommandHandlerTests
{
    ...
    private readonly ConfirmUserAddressCommandHandler _handler;

    public ConfirmUserAddressCommandHandlerTests()
    {
        _handler = new ConfirmUserAddressCommandHandler(
    ...
    {
        var command = new ConfirmUserAddressCommand(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnConfirmUserAddressFailed()
    {
        ...
        _shippingClient.ConfirmUserAddressAsync("token-1", "ifood-order-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodShippingActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodShipping.ConfirmUserAddressFailed");
    }
