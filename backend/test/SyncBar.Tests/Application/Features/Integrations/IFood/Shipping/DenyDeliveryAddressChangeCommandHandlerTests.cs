...
public sealed class DenyDeliveryAddressChangeCommandHandlerTests
{
    ...
    private readonly DenyDeliveryAddressChangeCommandHandler _handler;

    public DenyDeliveryAddressChangeCommandHandlerTests()
    {
        _handler = new DenyDeliveryAddressChangeCommandHandler(
    ...
    {
        var command = new DenyDeliveryAddressChangeCommand(1);
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnDenyAddressChangeFailed()
    {
        ...
        _shippingClient.DenyDeliveryAddressChangeAsync("token-1", "ifood-order-1", Arg.Any<CancellationToken>())
            .Returns(new IfoodShippingActionResult(false, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodShipping.DenyAddressChangeFailed");
    }
