...
public sealed class GetIfoodMerchantStatusByOperationQueryHandlerTests
{
    ...
    private readonly GetIfoodMerchantStatusByOperationQueryHandler _handler;

    public GetIfoodMerchantStatusByOperationQueryHandlerTests()
    {
        _handler = new GetIfoodMerchantStatusByOperationQueryHandler(
    ...
    {
        var query = new GetIfoodMerchantStatusByOperationQuery(1, "DELIVERY");
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnStatusByOperationFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodMerchantStatusByOperationQuery(1, "DELIVERY");
        _merchantClient.GetStatusByOperationAsync("token-1", "MERCH-1", "DELIVERY", Arg.Any<CancellationToken>())
            .Returns(new IfoodMerchantStatusByOperationResult(false, null, null, false, null, [], "erro remoto"));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodMerchant.StatusByOperationFailed");
    }

    [Fact]
    public async Task Handle_ValidRequest_ShouldReturnMappedStatus()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var query = new GetIfoodMerchantStatusByOperationQuery(1, "DELIVERY");
        _merchantClient.GetStatusByOperationAsync("token-1", "MERCH-1", "DELIVERY", Arg.Any<CancellationToken>())
            .Returns(new IfoodMerchantStatusByOperationResult(
                true, "DELIVERY", "ONLINE", true, "OK", [new IfoodMerchantValidation("v1", "OK", null)], null));

        var result = await _handler.Handle(query, CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        result.Value.Available.Should().BeTrue();
        result.Value.Validations.Should().ContainSingle(v => v.Id == "v1");
    }
}
