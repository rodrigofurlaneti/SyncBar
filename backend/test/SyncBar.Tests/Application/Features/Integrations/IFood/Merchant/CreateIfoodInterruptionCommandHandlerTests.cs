...
public sealed class CreateIfoodInterruptionCommandHandlerTests
{
    ...
    private readonly CreateIfoodInterruptionCommandHandler _handler;

    public CreateIfoodInterruptionCommandHandlerTests()
    {
        _handler = new CreateIfoodInterruptionCommandHandler(
    ...
    {
        var command = new CreateIfoodInterruptionCommand(1, "Manutenção", DateTime.UtcNow, DateTime.UtcNow.AddHours(1));
    ...
    [Fact]
    public async Task Handle_IfoodApiFails_ShouldReturnCreateInterruptionFailed()
    {
        var branch = CreateBranch();
        SetupResolvedMerchant(branch);
        var command = new CreateIfoodInterruptionCommand(1, "Manutenção", DateTime.UtcNow, DateTime.UtcNow.AddHours(1));
        _merchantClient.CreateInterruptionAsync("token-1", "MERCH-1", "Manutenção", command.Start, command.End, Arg.Any<CancellationToken>())
            .Returns(new IfoodCreateInterruptionResult(false, null, "erro remoto"));

        var result = await _handler.Handle(command, CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.Error.Code.Should().Be("IfoodMerchant.CreateInterruptionFailed");
    }
    ...
    public async Task Handle_ValidRequest_ShouldSucceed()
    {
        ...
        var command = new CreateIfoodInterruptionCommand(1, "Manutenção", DateTime.UtcNow, DateTime.UtcNow.AddHours(1));
        _merchantClient.CreateInterruptionAsync("token-1", "MERCH-1", "Manutenção", command.Start, command.End, Arg.Any<CancellationToken>())
            .Returns(new IfoodCreateInterruptionResult(true, "int-1", null));
