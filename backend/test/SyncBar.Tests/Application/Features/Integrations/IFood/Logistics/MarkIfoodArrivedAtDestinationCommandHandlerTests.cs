...
public sealed class MarkIfoodArrivedAtDestinationCommandHandlerTests
{
    ...
    private readonly MarkIfoodArrivedAtDestinationCommandHandler _handler;

    public MarkIfoodArrivedAtDestinationCommandHandlerTests()
    {
        _handler = new MarkIfoodArrivedAtDestinationCommandHandler(
    ...
    // Pré-requisito de MarkArrivedAtDestination: entrega em Dispatched.
    private static IfoodLogisticsDelivery CreateDeliveryDispatched()
    {
    ...
    {
        var command = new MarkIfoodArrivedAtDestinationCommand(IfoodOrderId: 1);
    ...
    {
        var order = CreateOrder();
        var delivery = CreateDeliveryDispatched();
        var command = new MarkIfoodArrivedAtDestinationCommand(IfoodOrderId: 1);
        _ifoodOrderRepository.GetByIdForUpdateAsync(command.IfoodOrderId, Arg.Any<CancellationToken>()).Returns(order);
    ...
    public async Task Handle_IfoodActionFails_ShouldReturnFailureWithoutTransitioning()
    {
        ...
        _logisticsClient.ArrivedAtDestinationAsync("token-1", order.IfoodOrderId, Arg.Any<CancellationToken>())
            .Returns(new IfoodLogisticsActionResult(false, "Falha ao registrar chegada no destino."));
    ...
    [Fact]
    public async Task Handle_DeliveryNotDispatched_ShouldReturnInvalidTransitionFailure()
    {
        var order = CreateOrder();
