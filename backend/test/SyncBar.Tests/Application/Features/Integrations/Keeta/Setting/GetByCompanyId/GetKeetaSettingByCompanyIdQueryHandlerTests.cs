...
public sealed class GetKeetaSettingByCompanyIdQueryHandlerTests
{
    ...
    private readonly GetKeetaSettingByCompanyIdQueryHandler _handler;

    public GetKeetaSettingByCompanyIdQueryHandlerTests()
    {
        _handler = new GetKeetaSettingByCompanyIdQueryHandler(_settingRepository, _logRepository, _unitOfWork);
    }

    [Fact]
    public async Task Handle_SettingNotFound_ShouldReturnNotFound()
    {
        _settingRepository.GetByCompanyIdAsync(1, Arg.Any<CancellationToken>()).Returns((KeetaIntegrationSetting?)null);

        var result = await _handler.Handle(new GetKeetaSettingByCompanyIdQuery(1), CancellationToken.None);
