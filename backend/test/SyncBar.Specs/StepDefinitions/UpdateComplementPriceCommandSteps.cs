...
public sealed class UpdateComplementPriceCommandSteps
{
    ...
    [When(@"eu tento atualizar o preco do complemento (.*) do grupo (.*) para (-?\d+)")]
    public async Task WhenEuTentoAtualizarOPrecoDoComplementoDoGrupoPara(long complementId, long groupId, decimal extraPrice)
    {
        // O Complement recem-adicionado em memoria tem Id sempre 0 (Entity via base(0) — nunca
        // persistido por um DbContext real neste teste). Resolve o id efetivo do unico
        // complemento ativo do grupo quando houver, senao usa o id informado no cenario (que nao
        // existira, gerando ComplementGroup.ComplementNotFound como esperado).
        var effectiveComplementId = _complementGroup?.Complements.FirstOrDefault(c => c.IsActive)?.Id ?? complementId;

        var handler = new UpdateComplementPriceCommandHandler(
            _complementGroupRepository.Object, _catalogSyncTrigger.Object, _logRepository.Object, _unitOfWork.Object);

        _result = await handler.Handle(new UpdateComplementPriceCommand(groupId, effectiveComplementId, extraPrice), CancellationToken.None);
    }
