...
[Binding]
[Scope(Feature = "Reverter versao do catalogo Ifood")]
public sealed class DowngradeIFoodCatalogVersionCommandSteps
{
    ...
    [Given(@"a reversao de versao do catalogo no Ifood falha com a mensagem ""(.*)""")]
    public void GivenAReversaoDeVersaoDoCatalogoNoIfoodFalhaComAMensagem(string errorMessage)
        => _catalogClient
            .Setup(c => c.DowngradeVersionAsync("token-1", "MERCH-1", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new IfoodCatalogActionResult(false, errorMessage));

    [Given(@"a reversao de versao do catalogo no Ifood tem sucesso")]
    public void GivenAReversaoDeVersaoDoCatalogoNoIfoodTemSucesso()
        => _catalogClient
            .Setup(c => c.DowngradeVersionAsync("token-1", "MERCH-1", It.IsAny<CancellationToken>()))
            .ReturnsAsync(new IfoodCatalogActionResult(true, null));

    [When(@"eu tento reverter a versao do catalogo Ifood da filial (.*)")]
    public async Task WhenEuTentoReverterAVersaoDoCatalogoIfoodDaFilial(long branchId)
    {
        var handler = new DowngradeIfoodCatalogVersionCommandHandler(
        ...
        _result = await handler.Handle(new DowngradeIfoodCatalogVersionCommand(branchId), CancellationToken.None);
    }
