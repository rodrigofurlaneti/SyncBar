...
internal sealed class GetUsersByCompanyQueryHandler(
    ...
    : BaseQueryHandler<GetUsersByCompanyQuery, IReadOnlyCollection<UserResponse>>(logRepository, unitOfWork)
{
    public override async Task<Result<IReadOnlyCollection<UserResponse>>> Handle(
        GetUsersByCompanyQuery request, CancellationToken cancellationToken)
    {
        return await ExecuteWithLogAsync(
            nameof(GetUsersByCompanyQueryHandler),
            nameof(Handle),
