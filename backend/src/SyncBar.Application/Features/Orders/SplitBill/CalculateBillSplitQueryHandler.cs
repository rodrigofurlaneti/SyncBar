...
internal sealed class CalculateBillSplitQueryHandler(
    ICustomerOrderRepository orderRepository,
    ILogTrackerRepository logRepository,
    IUnitOfWork unitOfWork)
    : BaseQueryHandler<CalculateBillSplitQuery, BillSplitResponse>(logRepository, unitOfWork)
{
    public override async Task<Result<BillSplitResponse>> Handle(CalculateBillSplitQuery request, CancellationToken cancellationToken)
    {
        return await ExecuteWithLogAsync(
            nameof(CalculateBillSplitQueryHandler),
            nameof(Handle),
