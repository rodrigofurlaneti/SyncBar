...
internal sealed class GetCashSessionHistoryQueryHandler(
    ...
    : BaseQueryHandler<GetCashSessionHistoryQuery, IReadOnlyCollection<CashSessionHistoryResponse>>(logRepository, unitOfWork)
{
    public override Task<Result<IReadOnlyCollection<CashSessionHistoryResponse>>> Handle(
        GetCashSessionHistoryQuery request, CancellationToken cancellationToken) =>
        ExecuteWithLogAsync(
            nameof(GetCashSessionHistoryQueryHandler),
            nameof(Handle),
