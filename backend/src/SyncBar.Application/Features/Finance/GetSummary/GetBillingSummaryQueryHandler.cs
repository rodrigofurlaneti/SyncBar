...
internal sealed class GetBillingSummaryQueryHandler(
    ...
    : BaseQueryHandler<GetBillingSummaryQuery, BillingSummaryResponse>(logRepository, unitOfWork)
{
    public override async Task<Result<BillingSummaryResponse>> Handle(
        GetBillingSummaryQuery request, CancellationToken cancellationToken)
    {
        return await ExecuteWithLogAsync(
            nameof(GetBillingSummaryQueryHandler),
            nameof(Handle),
