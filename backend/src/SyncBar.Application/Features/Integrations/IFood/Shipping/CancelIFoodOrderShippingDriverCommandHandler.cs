...
internal sealed class CancelIfoodOrderShippingDriverCommandHandler : BaseCommandHandler<CancelIfoodOrderShippingDriverCommand>
{
    ...
    public CancelIfoodOrderShippingDriverCommandHandler(
    ...
    public override async Task<Result> Handle(CancelIfoodOrderShippingDriverCommand request, CancellationToken cancellationToken)
    {
        return await ExecuteWithLogAsync(
            nameof(CancelIfoodOrderShippingDriverCommandHandler),
            nameof(Handle),
            null,
            async (userIdBox) =>
            {
                ...
                var result = await _shippingClient.CancelDriverForOrderAsync(token, IfoodOrder.IfoodOrderId, cancellationToken);
                if (!result.Success)
                    return Result.Failure(new Error("IfoodShipping.CancelDriverFailed", result.ErrorMessage ?? "Falha ao cancelar o entregador no Ifood."));
