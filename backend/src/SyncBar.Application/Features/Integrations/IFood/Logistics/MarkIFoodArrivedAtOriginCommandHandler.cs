...
internal sealed class MarkIFoodArrivedAtOriginCommandHandler : BaseCommandHandler<MarkIFoodArrivedAtOriginCommand>
{
    ...
    public MarkIFoodArrivedAtOriginCommandHandler(
    ...
    public override async Task<Result> Handle(MarkIFoodArrivedAtOriginCommand request, CancellationToken cancellationToken)
    {
        return await ExecuteWithLogAsync(
            nameof(MarkIFoodArrivedAtOriginCommandHandler),
            nameof(Handle),
            null,
            async (userIdBox) =>
            {
                ...
                var actionResult = await _logisticsClient.ArrivedAtOriginAsync(token, ifoodOrder.IFoodOrderId, cancellationToken);
                if (!actionResult.Success)
                    return Result.Failure(new Error("IFood.ActionFailed", actionResult.ErrorMessage ?? "Falha ao registrar chegada na origem no iFood."));

                var now = _timeProviderCustom.GetLocalNow().DateTime;
                var transition = delivery.MarkArrivedAtOrigin(now);
