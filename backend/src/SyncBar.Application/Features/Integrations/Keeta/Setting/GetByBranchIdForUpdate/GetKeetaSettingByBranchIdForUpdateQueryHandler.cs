...
{
    internal sealed class GetKeetaSettingByBranchIdForUpdateQueryHandler
        : BaseQueryHandler<GetKeetaSettingByBranchIdForUpdateQuery, KeetaIntegrationSettingResponse>
    {
        private readonly IKeetaIntegrationSettingRepository _settingRepository;

        public GetKeetaSettingByBranchIdForUpdateQueryHandler(
        ...
        public override async Task<Result<KeetaIntegrationSettingResponse>> Handle(
            GetKeetaSettingByBranchIdForUpdateQuery request,
            CancellationToken cancellationToken)
        {
            return await ExecuteWithLogAsync(
                nameof(GetKeetaSettingByBranchIdForUpdateQueryHandler),
                nameof(Handle),
                null,
                async (userIdBox) =>
                {
                    var setting = await _settingRepository.GetByBranchIdForUpdateAsync(request.BranchId, cancellationToken);
