...
{
    internal sealed class GetAllKeetaOrdersByCompanyIdQueryHandler
        : BaseQueryHandler<GetAllKeetaOrdersByCompanyIdQuery, IReadOnlyList<KeetaIntegrationOrderResponse>>
    {
        private readonly IKeetaIntegrationOrderRepository _orderRepository;

        public GetAllKeetaOrdersByCompanyIdQueryHandler(
        ...
        public override async Task<Result<IReadOnlyList<KeetaIntegrationOrderResponse>>> Handle(
            GetAllKeetaOrdersByCompanyIdQuery request,
            CancellationToken cancellationToken)
        {
            return await ExecuteWithLogAsync(
                nameof(GetAllKeetaOrdersByCompanyIdQueryHandler),
                nameof(Handle),
                null,
                async (userIdBox) =>
                {
                    var orders = await _orderRepository.GetAllByCompanyIdAsync(request.CompanyId, cancellationToken);
