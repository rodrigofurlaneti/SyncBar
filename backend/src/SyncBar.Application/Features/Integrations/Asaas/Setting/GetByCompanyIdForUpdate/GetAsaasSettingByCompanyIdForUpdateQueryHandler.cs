...
{
    internal sealed class GetAsaasSettingByCompanyIdForUpdateQueryHandler
        : BaseQueryHandler<GetAsaasSettingByCompanyIdForUpdateQuery, AsaasIntegrationSettingResponse>
    {
        private readonly IAsaasIntegrationSettingRepository _settingRepository;

        public GetAsaasSettingByCompanyIdForUpdateQueryHandler(
        ...
        public override async Task<Result<AsaasIntegrationSettingResponse>> Handle(
            GetAsaasSettingByCompanyIdForUpdateQuery request,
            CancellationToken cancellationToken)
        {
            return await ExecuteWithLogAsync(
                nameof(GetAsaasSettingByCompanyIdForUpdateQueryHandler),
                nameof(Handle),
                null,
                async (userIdBox) =>
                {
                    var setting = await _settingRepository.GetByCompanyIdForUpdateAsync(
                        request.CompanyId,
                        cancellationToken);
                    ...
                    if (setting is null)
                    {
                        return Result.Failure<AsaasIntegrationSettingResponse>(
                            Error.NotFound(
                                ...
                                $"Configuração global de integração Asaas não encontrada para atualização na empresa {request.CompanyId}."));
                    }
