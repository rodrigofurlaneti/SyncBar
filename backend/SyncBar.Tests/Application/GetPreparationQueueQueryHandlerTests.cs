...
public sealed class GetPreparationQueueQueryHandlerTests
{
    ...
    private GetPreparationQueueQueryHandler CreateHandler()
        => new(_orderRepository, _productRepository, _diningTableRepository, _comandaRepository);
    ...
    public async Task Handle_ShouldBuildTicketsWithKitchenTimeAndBarTolerance()
    {
        ...
        order.AddItem(1, 14.90m, 2, null, null);  // cerveja — sem tempo de preparo → BAR (5 min)
        order.AddItem(2, 32m, 1, "sem sal", null); // batata — 20 min de cozinha
